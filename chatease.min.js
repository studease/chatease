chatease=function(){if(chatease.api)return chatease.api.getInstance.apply(this,arguments)};chatease.version="0.1.18";chatease.debug=!1;
...
g.extend(this,new e.eventdispatcher("renders.def")),w={skin:{name:a.DEFAULT},prefix:"chat-"},m,k,q,l,v,p,r,t;h.show=function(a,b){var c=g.createElement("div"),r;switch(g.typeOf(a)){case "object":r=a.text;if("uni"==a.type){var t=g.createElement("span");t.innerHTML="[\u5bc6\u8bed]";c.appendChild(t)}break;default:r=a}switch(g.typeOf(b)){case "string":if(""===b)break;b={id:0,name:b,role:0};case "null":b={id:0,name:"[\u7cfb\u7edf]",role:64};case "object":if(null==g.typeOf(b.id))break;if(t=0<=b.role&&b.role&
64?"system":b.id==d.user().id?"myself":"")c.className=t;if(t=f(b.role)){var n=g.createElement("span",t);n.innerHTML=t.substr(0,1);c.appendChild(n)}t=g.createElement("a");t.user=g.extend({},b);t.innerHTML=b.name;try{t.addEventListener("click",function(a){h.dispatchEvent(e.CHATEASE_VIEW_NICKCLICK,{user:this.user})})}catch(k){t.attachEvent("onclick",function(a){h.dispatchEvent(e.CHATEASE_VIEW_NICKCLICK,{user:this.user})})}c.appendChild(t)}c.insertAdjacentHTML("beforeend",r);q.childNodes.length>=h.config.maxRecords&&
q.removeChild(q.childNodes[0]);q.appendChild(c);q.scrollTop=q.scrollHeight};h.send=function(){for(var a=0;a<h.config.channel.length;a++)h.dispatchEvent(e.CHATEASE_VIEW_SEND,{data:{text:r.value,type:"multi",channel:h.config.channel[a]}});h.clearInput()};h.clearInput=function(){r.value=""};h.clearScreen=function(){g.emptyElement(q)};h.element=function(){return m};h.resize=function(a,b){a=a||m.offsetWidth||c.width;b=b||m.offsetHeight||c.height;t&&t.resize(a,b)};h.destroy=function(){};h.config=g.extend({},
w,c);m=g.createElement("div","render");k=g.createElement("div","title");w=g.createElement("div","main");m.appendChild(k);m.appendChild(w);q=g.createElement("div","console");l=g.createElement("div","dialog");w.appendChild(q);w.appendChild(l);v=g.createElement("div","control");p=g.createElement("div","input");l.appendChild(v);l.appendChild(p);k.innerHTML="\u804a\u5929\u5ba4";(function(a,b,c,d,e){a=g.createElement("div",a);var r=g.createElement("label","ch"),f=g.createElement("input");f.type="checkbox";
f.checked=!!e;try{f.addEventListener("change",function(a){h.dispatchEvent(b,g.extend({shield:f.checked},c))})}catch(t){f.attachEvent("onchange",function(a){h.dispatchEvent(b,g.extend({shield:f.checked},c))})}r.appendChild(f);e=g.createElement("a");e.innerHTML=d;r.appendChild(e);a.appendChild(r);v.appendChild(a)})("msgshield",e.CHATEASE_VIEW_SHIELDMSG,null,"\u5c4f\u853d\u6d88\u606f",!1);(function(a,b,c,d,e){a=g.createElement("div",a);e=g.createElement("a","btn");e.innerHTML=d;try{e.addEventListener("click",
function(a){h.dispatchEvent(b,c)})}catch(r){e.attachEvent("onclick",function(a){h.dispatchEvent(b,c)})}a.appendChild(e);v.appendChild(a)})("clrscreen",e.CHATEASE_VIEW_CLEARSCREEN,null,"\u6e05\u5c4f","glyphicon glyphicon-trash");r=g.createElement("textarea");r.setAttribute("placeholder","\u8f93\u5165\u804a\u5929\u5185\u5bb9");h.config.maxlength&&r.setAttribute("maxlength",h.config.maxlength);try{r.addEventListener("keypress",n)}catch(y){r.attachEvent("onkeypress",n)}p.appendChild(r);k=g.createElement("button");
k.innerHTML="\u53d1\u9001";try{k.addEventListener("click",u)}catch(y){k.attachEvent("onclick",u)}p.appendChild(k);m.id=h.config.prefix+"render";w.id=h.config.prefix+"main";q.id=h.config.prefix+"console";l.id=h.config.prefix+"dialog";r.id=h.config.prefix+"input";k.id=h.config.prefix+"submit";try{t=new b[h.config.skin.name](h.config)}catch(y){g.log("Failed to init skin["+h.config.skin.name+"].")}t||h.dispatchEvent(e.CHATEASE_RENDER_ERROR,{message:"No suitable skin found!",skin:h.config.skin.name})}})(chatease);
...
d):(e=e.getPunishment(),null!=e&&0<(e.code&2)?n(403,d):l.send(JSON.stringify(d)))}else c()};k.join=function(a){a={cmd:"join",channel:{id:a}};var b=g.getCookie("token");b&&(a.token=b);k.send(a)};k.close=function(){l&&l.close()};a.addEventListener(e.CHATEASE_STATE,function(c){switch(c.state){case b.CONNECTED:d.show("\u804a\u5929\u5ba4\u5df2\u8fde\u63a5\u2026");p=0;k.dispatchEvent(e.CHATEASE_CONNECT);for(c=0;c<a.channel.length;c++)k.join(a.channel[c]);break;case b.CLOSED:d.show("\u804a\u5929\u5ba4\u8fde\u63a5\u5df2\u65ad\u5f00\uff01");
...
(function(f){var g=f.utils,e=f.embed,b=f.core.renders.modes,a=f.core.renders.skins.modes;e.config=function(d){d=g.extend({},{url:"ws://"+window.location.host+"/websocket/websck",width:300,height:450,channel:1,maxlength:30,maxRetries:0,retryDelay:3E3,render:{name:b.DEFAULT,skin:{name:a.DEFAULT}},keywords:"",maxRecords:50},d);switch(g.typeOf(d.channel)){case "array":break;case "number":d.channel=[d.channel];break;case "string":d.channel=d.channel.split(",");break;default:d.channel=[1]}return d};e.config.addConfig=
function(a,b){return g.extend(a,b)}})(chatease);(function(f){var g=f.utils,e=f.events,b=f.core;f.embed.embedder=function(a,d){function c(a){f.dispatchEvent(a.type,a)}var f=g.extend(this,new e.eventdispatcher("embed.embedder"));f.embed=function(){var e=new b.entity(d);e.addGlobalListener(c);e.setup();a.setEntity(e,d.render.name)}}})(chatease);