chatease=function(){if(chatease.api)return chatease.api.getInstance.apply(this,arguments)};chatease.version="0.1.14";chatease.debug=!1;
...
(function(d){var f=d.utils,e=d.events;d=d.core;var b=d.states;d.controller=function(a,c){function d(){if(!m){try{m=window.WebSocket?new WebSocket(a.url):window.MozWebSocket?new MozWebSocket(a.url):new SockJS(a.url.replace(/^ws/,"http")+"/sockjs")}catch(c){f.log("Failed to initialize websocket.");return}m.onopen=function(c){a.setState(b.CONNECTED)};m.onmessage=h;m.onerror=function(c){a.setState(b.ERROR)};m.onclose=function(c){a.setState(b.CLOSED)}}}function h(b){var d;try{d=eval("("+b.data+")")}catch(g){f.log("Eval failed. \nerr: "+
g+"\ndata: "+b.data);return}switch(d.raw){case "ident":f.foreach(d.user,function(b,c){a.user.hasOwnProperty(b)&&(a.user[b]=c)});a.interval=d.user.interval;c.show("\u52a0\u5165\u623f\u95f4\u6210\u529f\uff01");l.dispatchEvent(e.CHATEASE_INDENT,d);break;case "message":try{r||(r=new f.filter(a.keywords)),d.text=r.replace(d.text)}catch(g){f.log("Failed to execute filter.")}c.show(d,d.user);l.dispatchEvent(e.CHATEASE_MESSAGE,d);break;case "join":c.show(x(d.user.role)+" "+d.user.name+" \u8fdb\u5165\u804a\u5929\u5ba4\u3002");
l.dispatchEvent(e.CHATEASE_JOIN,d);break;case "left":c.show(x(d.user.role)+" "+d.user.name+" \u5df2\u79bb\u5f00\u3002");l.dispatchEvent(e.CHATEASE_LEFT,d);break;case "error":(b=t(d))&&c.show(b);l.dispatchEvent(e.CHATEASE_ERROR,d);break;default:f.log("Unknown data type, ignored.")}}function t(a){var b;switch(a.error.code){case 400:b="\u9519\u8bef\u8bf7\u6c42\uff01";break;case 401:b="\u8bf7\u5148\u767b\u5f55\uff01";break;case 403:b="\u6743\u9650\u9519\u8bef\uff01";break;case 404:b="\u672a\u77e5\u8bf7\u6c42\uff01";
break;case 409:b="\u64cd\u4f5c\u9891\u7e41\uff01"}return b}function x(a){var b="";switch(f.typeOf(a)){case "string":if(a=parseInt(a),NaN==a||0>a)break;case "number":a&192?b="\u8d85\u7ba1":a&32?b="\u4e3b\u64ad":a&24?b="\u623f\u7ba1":a&7&&(b="VIP"+(a&7))}return b}function k(){if(!a.maxRetries||p<a.maxRetries){var b=Math.ceil(a.retryDelay+5E3*Math.random());c.show("\u6b63\u5728\u51c6\u5907\u91cd\u8fde\uff0c"+b/1E3+"\u79d2...");setTimeout(function(){p++;m=null;d()},b)}}function u(a){l.dispatchEvent(a.type,
a)}var l=f.extend(this,new e.eventdispatcher("core.controller")),q=!1,m,r,w=0,p=0;l.send=function(e){if(m&&a.state!=b.CLOSED){var f=(new Date).getTime();0<=a.interval&&f-w<a.interval?c.show("\u64cd\u4f5c\u9891\u7e41\uff01"):(w=f,m.send(JSON.stringify(e)))}else d()};l.join=function(a){a={cmd:"join",channel:{id:a}};var b=f.getCookie("token");b&&(a.token=b[1]);l.send(a)};l.close=function(){m&&m.close()};a.addEventListener(e.CHATEASE_STATE,function(d){switch(d.state){case b.CONNECTED:c.show("\u804a\u5929\u5ba4\u5df2\u8fde\u63a5\u2026");
